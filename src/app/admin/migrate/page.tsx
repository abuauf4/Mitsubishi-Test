...
export default function MigratePage() {
  ...
  const migrate = async (step: string = 'all') => {
    if (!confirm(`Run migration (${step})? This will modify your blob store and database.`)) return;
    setLoading(true);
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-4 md:p-8">
      <div className="max-w-3xl mx-auto space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold text-white">🗄️ Database & Blob Migration</h1>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">Step 1: Scan Status</h2>
            <button
              onClick={scan}
              disabled={scanning}
          ...
          {scanStatus && (
            <div className="space-y-3">
              ...
              {/* Database status */}
              <div className="bg-gray-800/50 rounded-lg p-3">
                <h3 className="text-sm font-medium text-gray-300 mb-2">Turso Database</h3>
                {scanStatus.database.error ? (
                  <p className="text-red-400 text-sm">❌ {scanStatus.database.error}</p>
                ) : (
                  <div className="space-y-1">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">🔒 Private URLs in DB</span>
                      <span className={scanStatus.database.privateUrls ? 'text-yellow-400' : 'text-green-400'}>
                        {scanStatus.database.privateUrls ?? 0}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">🔀 Proxy URLs in DB</span>
                      <span className={scanStatus.database.proxyUrls ? 'text-yellow-400' : 'text-green-400'}>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 space-y-3">
          ...
          <p className="text-gray-400 text-xs">Copy private blobs → public, then update database URLs</p>
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            >
              🔍 <strong>Dry Run</strong> — Preview changes without applying
            </button>

            <button
              onClick={() => migrate('all')}
              disabled={loading}
              className="px-4 py-3 bg-red-700 hover:bg-red-600 disabled:bg-gray-900 rounded-lg text-sm font-medium transition-colors text-white"
            >
              🚀 <strong>Full Migration</strong> — Copy blobs + Update DB URLs
            </button>

            <button
              onClick={() => migrate('urls')}
              disabled={loading}
              className="px-4 py-3 bg-amber-700 hover:bg-amber-600 disabled:bg-gray-900 rounded-lg text-sm font-medium transition-colors text-white"
            >
              🔗 <strong>Fix Proxy URLs Only</strong> — Update /api/image?url= → direct public URLs
            </button>

            <button
              onClick={() => migrate('db')}
              disabled={loading}
              className="px-4 py-3 bg-blue-700 hover:bg-blue-600 disabled:bg-gray-900 rounded-lg text-sm font-medium transition-colors text-white"
            >
              📝 <strong>Update DB URLs Only</strong> — Fix private → public URLs in database
            </button>
          </div>
        </div>
